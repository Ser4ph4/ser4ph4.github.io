  ...
  // --- Lógica do Formulário de Contato (presente apenas em contact.html) ---
    ...
    if (contactForm) { // Só executa se estiver na página contact.html
        contactForm.addEventListener('submit', function(event) {
            ...
            formMessage.textContent = 'Transmissão enviada! Em breve entraremos em contato.';
            formMessage.classList.add('success');

            // Limpa o formulário após o "envio"
            contactForm.reset();

            // --- IMPORTANTE: Lógica de envio para o backend ---
            // Para enviar o e-mail de verdade, você precisaria de um backend.
            // Exemplos:
            // 1. Usar um serviço como Formspree (https://formspree.io/):
            //    Você mudaria a action do seu <form> para a URL do Formspree.
            //    Ex: <form action="https://formspree.io/f/your_form_id" method="POST">
            // 2. Criar um servidor backend (Node.js com Express e Nodemailer, Python com Flask/Django, PHP, etc.)
            //    E faria uma requisição Fetch/XMLHttpRequest para enviar os dados.
            /*
            fetch('/api/send-email', { // Exemplo de endpoint no seu backend
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    formMessage.textContent = 'Erro ao enviar transmissão. Tente novamente mais tarde.';
                    formMessage.classList.add('error');
                }
            })
            .catch(error => {
                console.error('Erro:', error);
                formMessage.textContent = 'Erro de rede. Verifique sua conexão e tente novamente.';
                formMessage.classList.add('error');
            });
            */
        });
